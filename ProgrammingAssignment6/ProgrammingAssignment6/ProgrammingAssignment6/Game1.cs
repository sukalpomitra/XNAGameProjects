...
namespace ProgrammingAssignment6
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            playerHand[0].X = TOP_CARD_OFFSET;
            playerHand[0].Y = TOP_CARD_OFFSET;
            playerHand[0].FlipOver();

            // first dealer card
            dealerHand.Add(deck.TakeTopCard());
            dealerHand[0].X = TOP_CARD_OFFSET + HORIZONTAL_MENU_BUTTON_OFFSET +  HORIZONTAL_CARD_OFFSET;
            dealerHand[0].Y = TOP_CARD_OFFSET;

            //// second player card
            playerHand.Add(deck.TakeTopCard());
            playerHand[1].X = TOP_CARD_OFFSET;
            playerHand[1].Y = TOP_CARD_OFFSET + VERTICAL_CARD_SPACING;
            playerHand[1].FlipOver();

            //// second dealer card
            dealerHand.Add(deck.TakeTopCard());
            dealerHand[1].X = TOP_CARD_OFFSET +  HORIZONTAL_MENU_BUTTON_OFFSET + HORIZONTAL_CARD_OFFSET;
            ...
            switch (currentState)
            {
                case GameState.CheckingHandOver:
                    if (GetBlackjackScore(playerHand) > MAX_HAND_VALUE || GetBlackjackScore(dealerHand) > MAX_HAND_VALUE || (!playerHit && !dealerHit))
                    {
                        ...
                        messageFont,
                        new Vector2(TOP_CARD_OFFSET + HORIZONTAL_MESSAGE_OFFSET + HORIZONTAL_MENU_BUTTON_OFFSET, SCORE_MESSAGE_TOP_OFFSET));
                        messages.Add(dealerScoreMessage);
                        menuButtons.Clear();
                        menuButtons.Add(new MenuButton(quitButtonSprite, new Vector2(HORIZONTAL_MENU_BUTTON_OFFSET, TOP_MENU_BUTTON_OFFSET + (3 * VERTICAL_MENU_BUTTON_SPACING)), GameState.Exiting));
                        if (GetBlackjackScore(playerHand) > MAX_HAND_VALUE && GetBlackjackScore(dealerHand) > MAX_HAND_VALUE)
                            winningMessage = new Message(TIE, messageFont, winnerMessageLocation);
                        else if (GetBlackjackScore(playerHand) <= MAX_HAND_VALUE && GetBlackjackScore(dealerHand) <= MAX_HAND_VALUE)
                        {
                            ...
                            {
                                winningMessage = new Message(PLAYER_WINS, messageFont, winnerMessageLocation);
                            }
                            else
                            {
                                winningMessage = new Message(DEALER_WINS, messageFont, winnerMessageLocation);
                            }
                        }
                ...
                case GameState.DealerHitting:
                    ...
                    dealerCard.X = TOP_CARD_OFFSET + HORIZONTAL_MENU_BUTTON_OFFSET + HORIZONTAL_CARD_OFFSET;
                    dealerCard.Y = TOP_CARD_OFFSET + (dealerHandSize * VERTICAL_CARD_SPACING);
                    dealerCard.FlipOver();
                ...
                case GameState.PlayerHitting:
                    ...
                    playerCard.X = TOP_CARD_OFFSET;
                    playerCard.Y = TOP_CARD_OFFSET + (playerHandSize * VERTICAL_CARD_SPACING);
                    playerCard.FlipOver();
